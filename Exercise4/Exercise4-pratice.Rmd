...
```{r ans12}
stringr::str_which(names(gsHW2ws),"ans")
```
...
allBankData %>%
  mutate(
    銀行=as.factor(銀行),
    `定存利率-三個月-固定`=parse_double(`定存利率-三個月-固定`,locale=locale(decimal_mark = ".")),
    `定存利率-一年期-固定`=parse_double(`定存利率-一年期-固定`,locale=locale(decimal_mark = "."))
  )->allBankData
```
...
exData %>%
  mutate_at(
    ...
    funs(
      parse_number(.,na=c("-","","NA"),locale = locale(decimal_mark = "."))
    )
  )%>%
    mutate(
        期間 = str_replace(期間,"M","-"),
...
```{r ans33}
colnames(exData[2:19]) %>%
  str_locate(.,"/") %>%
  as.data.frame(.) %>%
  {.[(1:18),1]-4} %>%
  str_sub(colnames(exData[2:19]),1,.)->colnames(exData)[2:19]
exData
