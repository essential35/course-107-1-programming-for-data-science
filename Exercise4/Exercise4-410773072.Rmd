...
params:
  studentID: "410773072"
  studentName: "張翊宣"
  repoURL: "https://github.com/essential35/course-107-1-programming-for-data-science"
---
...
```{r ans14}
gsHW2ws$答對率<-rowMeans(gsHW2ws[1:5],na.rm = T)
```
...
```{r ans16}
stringr::str_which(gsHW2ws$成績,"9") %>%
  as.character(.) %>%
  gsHW2ws[.,1:5] %>% 
  colSums(.) %>%
 { which(. == min(.))} %>%
...
```{r ans17}
stringr::str_which(gsHW2ws$成績,"8") %>%
  as.character(.) %>%
  gsHW2ws[.,1:5] %>% 
  colMeans(.) %>%
  {. <= "0.5"} %>%
...
allBankData %>%
  mutate(
    銀行=as.factor(銀行),
    `定存利率-三個月-固定`=parse_double(`定存利率-三個月-固定`,na=("定存利率-三個月-固定"),locale = cl),
    `定存利率-一年期-固定`=parse_double(`定存利率-一年期-固定`,na=("定存利率-一年期-固定"),locale = cl)
  )->allBankData
```
...
exData %>%
  mutate_at(
    vars(`印尼盾IDR/USD`,`歐元USD/EUR`,`馬克DEM/USD`,`法國法郎FRF/USD`,`荷蘭幣NLG/USD`,`越南盾VND/USD`),
    funs(parse_double(.,na=("-"),locale = cl))
  ) ->exData
...
```{r ans33}
library(stringr)
str_locate(colnames(exData[2:19]),"/") %>%
    as.data.frame(.) %>%
  {.[1:18,1]-4} %>%
  as.vector(.) %>%
  {str_sub(colnames(exData[2:19]),1,c(.))}->colnames(exData)[2:19]
exData
```
