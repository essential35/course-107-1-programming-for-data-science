...
totalCredits <- function(x){
  id <- x
  subsample1<- transcriptData[(transcriptData$學號 == id),]
  twant <- sum(subsample1$學分數)
  subsample1$學期成績 >= 60 -> subsample1$及格
  tget <- sum(subsample1$學分數[subsample1$及格])
  paste0("學號:",id," 總修習學分數:",twant," 總實得學分數:",tget)->result
  return(result)
}
...
```{r}
x<-seq_along(transcriptData$學號)
x
#map(c(1,2...,2803),function(x))   x=1的值帶入function，得到的值回存
i<-2
totalCreditsOfStudent_i<-function(i){
  transcriptData$學號[i] %>% totalCredits
}


#map(x,totalCreditsOfStudent_i)
#map(transcriptData$學號,totalCredits)
#把學號丟回totalCredits的函數內，因為此函數可以接到學號變數得出答案
...
```{r ans18}
transcriptData %>% 
{(.$學年 == "104") & (.$學期 == "1") & (.$科目名稱 == "ZT13")} %>%
  transcriptData[.,] -> sample1

shannon(c("D8","G8","H1","J4","L6","M6","N3","N8","O3","P1","Q7","Q9","S3","V1","W5","W7"))
...
##challenge
在transcriptData創新的logic變數**每週一第六節有課**
```{r}
str_replace(transcriptData$上課時間及教室,"~",":")->transcriptData$上課時間及教室
str_c("c(",transcriptData$上課時間及教室,")")->transcriptData$上課時間及教室
```
