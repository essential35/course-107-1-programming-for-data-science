...
```{r ans16}
transcriptData %>%
  filter(`必選修類別（必∕選∕通）`=="通",學年=="100",學期=="1") %>%
  .$科目名稱 %>% unique ->通識科目總表
```
...
```{r}
str_replace_all(transcriptData$上課時間及教室,"~",":")->transcriptData$上課時間及教室
str_c("c(",transcriptData$上課時間及教室,")") %>% as.factor ->transcriptData$上課時間及教室
