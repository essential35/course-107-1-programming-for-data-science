...
student1 %>% 
  filter(.$data=="list(學期 = 1:2)") ->student1 
student1 %>%
  group_by(學號,年級) %>% summarise %>% nest ->student1
student1 %>%
  mutate(logic=str_detect(.$data," 1:4")) %>%
  filter(logic=="TRUE") ->student1
length(student1$學號)
#算出來有2342人
...
transcriptData$學年學期 %>% as.factor->a 
1:length(levels(a))->levels(transcriptData$學年學期)
as.integer(transcriptData$學年學期)
