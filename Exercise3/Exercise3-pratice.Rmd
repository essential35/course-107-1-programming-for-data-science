...
classData %>%
  mutate(
    `公-10-06` = as.factor(`公-10-06`),
    `公-10-06` = str_replace(`公-10-06`,"1","有看公告"),
    `公-10-06` = str_replace(`公-10-06`,"0","沒看公告")
  )->classData
```
...
```{r ans12}
classData[(classData$`公-10-06` == "有看公告"),]->yesclassData
```
...
```{r ans13}
sum(yesclassData$`練2018-10-17-1`)/nrow(yesclassData)
```
...
```{r ans14}
classData[(classData$`公-10-06` == "沒看公告"),] %>%
  {sum(.$`練2018-10-17-1`)/nrow(.)}
```
...
```{r ans23}
difftime(deadline,hwData$Timestamp,units="day")->hwData$early
```
...
```{r ans25}
hwData[(hwData$Timestamp < deadline & hwData$作業代碼 == "hw1"),3] %>%
  {table(.$early)}
```
